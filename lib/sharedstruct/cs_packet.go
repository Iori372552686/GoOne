...
func (h *CSPacketHeader) From(b []byte) {
	pos := 0
	h.Version = binary.BigEndian.Uint16(b[pos:]); pos += 2
	h.PassCode = binary.BigEndian.Uint16(b[pos:]); pos += 2
	h.Seq = binary.BigEndian.Uint32(b[pos:]); pos += 4
	h.Uid = binary.BigEndian.Uint64(b[pos:]); pos += 8
	h.AppVersion = binary.BigEndian.Uint32(b[pos:]); pos += 4
...
func (h *CSPacketHeader) To(b []byte) {
	pos := uintptr(0)
	binary.BigEndian.PutUint16(b[pos:], h.Version); pos += 2
	binary.BigEndian.PutUint16(b[pos:], h.PassCode); pos += 2
	binary.BigEndian.PutUint32(b[pos:], h.Seq); pos += 4
	binary.BigEndian.PutUint64(b[pos:], h.Uid); pos += 8
	binary.BigEndian.PutUint32(b[pos:], h.AppVersion); pos += 4
