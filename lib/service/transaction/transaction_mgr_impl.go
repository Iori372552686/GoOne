...
func (m *TransactionMgr) RegisterCmd(cmd uint32, cmdHandler cmd_handler.ICmdHandler) {
	...
	if nil == m.cmdHandlers {
		m.cmdHandlers = make(map[uint32]cmd_handler.ICmdHandler, 0)
	}
...
func (m *TransactionMgr) processSSPacket(packet *sharedstruct.SSPacket) int32 {
	...
	logger.Infof("get cmd %#v", packet.Header)

	if dstTransID == 0 {
		if m.useUidLock && m.uidInProcess[uid] { // pending or drop
			packets := m.uidPendingPackets[uid]
			if packets == nil {
		...
		if !in {
			logger.Errorf("no cmd {cmd:0x%x}", cmd)
			return -2
		}
...
func (m *TransactionMgr) processTransactionRet(aTransRet transRet) {
	trans, in := m.transMap[aTransRet.transID]
	if !in || trans == nil {
	...
	uid := trans.Uid()
	close(trans.chanIn)
	delete(m.transMap, aTransRet.transID)
	delete(m.uidInProcess, uid)

	// 这里如果uid对应的队列里面还有其他未处理的包，则继续处理
	if m.useUidLock {
		if packets, in := m.uidPendingPackets[uid]; in {
			if len(packets) <= 0 {
				delete(m.uidPendingPackets, uid)
			} else {
				p := packets[0]
				m.uidPendingPackets[uid] = packets[1:]
				m.processSSPacket(p)
			}
		}
	}
