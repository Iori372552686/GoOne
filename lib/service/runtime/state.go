...
//
// 重入限制：transition 用 transitionMu 串行化整个迁移序列，派发观察者时仍持有
...
type StateStore struct {
	...
	// transitionMu 串行化完整的状态迁移序列"校验 → observer → 提交"（修复）。
...
//
// 关键不变量（修复）：构造即上报，使指标在进程启动瞬间就反映 starting，
// 而非等到第一次状态转换才翻转。
...
//
// 串行化（修复）：transitionMu 覆盖完整"校验 → observer → 提交"序列，确保两个
...
//
// 指标时机（修复）：goone_lifecycle_state gauge 的翻转发生在状态**提交成功之
...
//
// startedAt 由 StateStore 自身持有（构造时记录），admin 不再接收 App 的第二
