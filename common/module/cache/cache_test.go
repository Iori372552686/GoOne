...
func TestTouch(t *testing.T) {
	...
	tc.Set("e", 5, 20*time.Millisecond)
	<-time.After(10 * time.Millisecond)
	succ := tc.Touch("e", 20*time.Millisecond)
	require.True(t, succ)
	<-time.After(10 * time.Millisecond)
	_, found = tc.Get("e")
	if !found {
		t.Error("Did not find e even though it was set a new expiration")
	}
	<-time.After(15 * time.Millisecond)
	_, found = tc.Get("e")
	if found {
		t.Error("Found e when it should have been automatically deleted")
	}
}
...
func TestCache_Has(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	tc.Set("foo", "bar", DefaultExpiration)
	require.True(t, tc.Has("foo"))
	require.False(t, tc.Has("bar"))
}

func TestCache_Pop(t *testing.T) {
	tc := New(DefaultExpiration, 0)
	_, deleted := tc.Pop("foo")
